// The MACE decoder has been ported from the FFmpeg project.
// This file is based on "libavcodec/mace.c", as it was
// in FFmpeg stable release 4.3.1.
//
// The functionality has been reduced to just what Glider PRO
// needs, so stereo sound support has been removed.
...
static MACEtab1: [i16; 8] = [-13, 8, 76, 222, 222, 76, 8, -13];

static MACEtab3: [i16; 4] = [-18, 140, 140, -18];

#[rustfmt::skip]
static MACEtab2: [i16; 4 * 128] = [
...
const fn QT_8S_2_16S(x: i16) -> i16 {
    let tmp = x as u16;
    let tmp = (tmp & 0xFF00) | ((tmp >> 8) & 0xFF);
    tmp as i16
}
...
fn read_table(chd: &mut ChannelData, val: u8, tab_idx: usize) -> i16 {
    ...
    let base_index = ((chd.index & 0x7f0) as usize >> 4) * tabs[tab_idx].stride;

    let current = if val < tabs[tab_idx].stride {
        tabs[tab_idx].tab2[base_index + val]
    } else {
        -1 - tabs[tab_idx].tab2[base_index + 2 * tabs[tab_idx].stride - val - 1]
    };

    chd.index += tabs[tab_idx].tab1[val] - (chd.index >> 5);
    if chd.index < 0 {
        chd.index = 0;
    }
...
fn chomp3(chd: &mut ChannelData, val: u8, tab_idx: usize) -> [i16; 1] {
    let mut current = read_table(chd, val, tab_idx);

    current = mace_broken_clip_int16(i32::from(current) + i32::from(chd.level));
...
fn chomp6(chd: &mut ChannelData, val: u8, tab_idx: usize) -> [i16; 2] {
    let mut current = read_table(chd, val, tab_idx);

    if (chd.previous ^ current) >= 0 {
        chd.factor = cmp::min(i32::from(chd.factor) + 506, 32767) as i16;
    } else {
        if i32::from(chd.factor) - 314 < -32768 {
            chd.factor = -32767;
        } else {
            chd.factor -= 314;
        }
    ...
    chd.level = ((i32::from(current) * i32::from(chd.factor)) >> 15) as i16;
    current >>= 1;

    let output = [
        QT_8S_2_16S(chd.previous + chd.prev2 - ((chd.prev2 - current) >> 2)),
        QT_8S_2_16S(chd.previous + current + ((chd.prev2 - current) >> 2)),
    ];
...
pub fn mace3_decode_mono(buf: &[u8]) -> io::Result<Vec<i16>> {
    if (buf.len() % 2) != 0 {
        return Err(Error::new(ErrorKind::InvalidData, "MACE3 buffer size is odd"));
    }
    let mut chd = ChannelData::default();
    let mut samples = Vec::with_capacity(3 * buf.len());
    for &pkt in buf {
        let val = [pkt & 7, (pkt >> 3) & 3, pkt >> 5];
        for idx in 0..3 {
            let output = chomp3(&mut chd, val[idx], idx);
...
pub fn mace6_decode_mono(buf: &[u8]) -> io::Result<Vec<i16>> {
    let mut chd = ChannelData::default();
    let mut samples = Vec::with_capacity(6 * buf.len());
    for &pkt in buf {
        let val = [pkt >> 5, (pkt >> 3) & 3, pkt & 7];
        for idx in 0..3 {
            let output = chomp6(&mut chd, val[idx], idx);
            samples.extend_from_slice(&output);
        }
    }
    Ok(samples)
